...
const HomeHero = () => {
  ...
  const localDate = clock
    ? dateFormatter.format(clock.now)
    : "Loading local date";
    ...
    <div className="h-[280px] w-full rounded-[20px] bg-hero bg-cover">
      <div className="flex h-full flex-col justify-between max-md:px-5 max-md:py-8 lg:p-9">
        <div className="flex flex-wrap items-center gap-3">
          <p className="glassmorphism rounded-full px-4 py-2 text-sm font-medium uppercase tracking-[0.2em] text-sky-3">
            Local Time
          </p>
        ...
        <div className="flex flex-col gap-2">
          <time
            dateTime={clock ? clock.now.toISOString() : undefined}
            aria-live="polite"
          ...
          {/* <p className="text-sm text-sky-2">
            Synced to your device clock so the dashboard always matches your
            local time.
          </p> */}
        </div>
      </div>
